...
def post_processing(DESTINATION: int, pi_label: dict, PRINT_ITINERARY: int, label: dict) -> tuple:
    ...
    if rounds_inwhich_desti_reached == []:
        ...
        if PRINT_ITINERARY == 1:
            df_path = _print_Journey_legs(pareto_set, True)
        #        _save_routesExplored(save_routes, routes_exp)
        return rounds_inwhich_desti_reached, trip_set, rap_out, df_path


def _print_Journey_legs(pareto_journeys: list, silent: bool) -> None:
    '''
    Prints journey in correct format. Parent Function: post_processing

    Args:
        pareto_journeys (list): pareto optimal set.
        mode (boolean): if True, return dataframe without printing output 
    ...
    for _, journey in pareto_journeys:
      # Collect path information in a DF
      choice += 1
      #
      # Print 
      for leg in journey:
        if leg[0] == 'walking':
          ent_leg = pd.DataFrame({
            'mode': ['walking'], 
            'origin': [leg[1]], 
            'destination': [leg[2]],
            'tt_seconds': [leg[3].total_seconds()],
            'arrival_time': [leg[4]],
            'route': [None],
            'choice': [choice]
          })
  ...
  #                print(f'from {leg[1]} walk till  {leg[2]} for {leg[3]} minutes and reach at {leg[4].time()}')
          ...
          ent_leg = pd.DataFrame({
            ...
            'origin': [leg[1]], 
            'destination': [leg[2]],
            'tt_seconds': [(leg[3] - leg[0]).total_seconds()],
            'arrival_time': [leg[3]],
            'route': [leg[4]],
            'choice': [choice]
          })
        ...
        df_path = pd.concat([df_path, ent_leg], ignore_index=True)
      if not silent:
        print("####################################")
    return df_path
