...
def extract_graph(NETWORK_NAME: str, READ_PATH: str) -> tuple:
    """
    Extracts the required OSM.
    Args:
        NETWORK_NAME (str): Network name, e.g., 'Atlanta, GA, USA'
...
def parallel_func(source_info: tuple, G, WALKING_LIMIT, stops_list) -> list:
    """
...
def post_process(transfer_file, WALKING_LIMIT: int, SAVE_PATH: str) -> None:
    """
    ...
    Args:
        ...
        SAVE_PATH (str): Saving location

    Returns:
        None
    """
    transfer_file.to_csv(f'{SAVE_PATH}/transfers.csv', index=False)
    transfer_file.to_csv(f'{SAVE_PATH}/transfers.txt', index=False)
